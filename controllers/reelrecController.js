...
exports.updateUser = async (req, res) => {
    try {
      const updatedUser = await ReelRec.findOneAndUpdate({ uid: req.params.uid }, req.body, { new: true });
  ...
  exports.deleteUser = async (req, res) => {
    try {
      const deletedUser = await ReelRec.findOneAndDelete({ uid: req.params.uid });
  ...
  exports.addCollection = async (req, res) => {
    try {
      const user = await ReelRec.findOne({ uid: req.params.uid });
      if (!user) return res.status(404).json({ message: 'User not found' });
